...
data_dir = "./env_test/ai_torch3/Project"
...
class RockScissorPaper(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if label_str.startswith("rock") == True:
            label = 0 
        elif label_str.startswith("scissor") == True:
            label =1
        else :
            label = 2
...
# data정의 data loader
train_data_set = RockScissorPaper(data_dir, mode="train", transform=data_transforms['train'])
val_data_set = RockScissorPaper(data_dir, mode='val', transform=data_transforms['val'])
test_data_set = RockScissorPaper(data_dir, mode='test', transform=data_transforms['test'])
...
# Vgg
model_ft = models.vgg11(pretrained=True).to(device)
#어떤 모델을 쓸것인지 고르고
...
lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=3, gamma=0.05)
...
def validation(epoch, model, data_loader, criterion, device):
    ...
    with torch.no_grad():
        ...
        print("Validation {} Accuracy {:.2f} % Average Loss : {:.4f}"
...
def save_model(model, save_dir, file_name="RockScissorPaper_vggg.pt"):
    ...
    # train start 
val_every = 1 
save_dir = "./env_test/ai_torch3"

train(num_epochs, model_ft, train_data_loader, criterion, optimizer, save_dir, val_every, device)